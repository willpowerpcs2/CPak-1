...
def get_csv_files(input_dir):
    ...
    for file in os.listdir(input_dir):
        if file.endswith('.csv'):
            ...
            metadata_file = os.path.join(input_dir, file)
    
    return csv_files, metadata_file

def get_table_name_from_csv(csv_file):
    """Get table name from CSV file name."""
    base_name = os.path.basename(csv_file)
    if base_name.startswith('query_'):
        # For query files, use the name after 'query_' and before '.csv'
        return base_name[6:-4]
    else:
        # For table files, use the name before '.csv'
        return base_name[:-4]
...
def main():
    ...
    # Get CSV files
    csv_files, metadata_file = get_csv_files(args.input)
